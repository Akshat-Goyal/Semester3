...
#include "shell.h"
	...
	int nightswatch(char **cmd, char num){			// executes nightwatch command after error handling
		int sec = 2;
		if(num < 2){
			printf("nightswatch: too less arguments\n");
			return 1;
		}
		if(cmd[1][0] == '-' && (strlen(cmd[1]) == 1 || cmd[1][1] != 'n')){
			printf("nightswatch: invalid option -- %s\n", cmd[1]);
			return 1;
		}
		else if(cmd[1][0] == '-'){
			if(strlen(cmd[1]) == 2 && num == 2){
				printf("nightswatch: option requires an argument -- %s\n", cmd[1]);
				return 1;
			}
			...
			if(sec == -1){
				printf("nightswatch: failed to parse argument\n");
				return 1;
			}
			if( start > num){
				printf("nightswatch: requires a <command>\n");
				return 1;
			}
			if(start != num || (strcmp(cmd[start-1], "interrupt") !=0 && strcmp(cmd[start-1], "dirty") !=0)){
				for(int i=0; i<num; i++){
					printf("%s ", cmd[i]);
				}
